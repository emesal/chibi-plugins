...
/// Path to the built binary — resolved at compile time by cargo.
const BIN: &str = env!("CARGO_BIN_EXE_lang_rust");

/// Helper: run lang_rust with JSON on stdin, return parsed output.
fn run_with_input(input: &serde_json::Value) -> serde_json::Value {
...
fn integration_basic_fixture() {
    let fixture =
        std::fs::read_to_string("tests/fixtures/basic.rs").expect("fixture file missing");
    ...
    let find_sym = |name: &str, kind: &str| {
        symbols
            .iter()
            .find(|s| s["name"].as_str().unwrap() == name && s["kind"].as_str().unwrap() == kind)
    };
    assert_eq!(
        find_sym("input", "field").unwrap()["parent"].as_str(),
        Some("Parser")
    );
    assert_eq!(
        find_sym("Word", "variant").unwrap()["parent"].as_str(),
        Some("Token")
    );
    assert_eq!(
        find_sym("helper", "function").unwrap()["parent"].as_str(),
        Some("internal")
    );
}
...
fn integration_schema_mode() {
    ...
    let schema: serde_json::Value =
        serde_json::from_slice(&output.stdout).expect("invalid schema JSON");
